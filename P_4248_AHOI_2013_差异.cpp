...
const int N = 2e6;
int n, num = 126, f1[N], f2[N], sa[N], tong[N], height[N];
char ch[N];
void SA() {
...
int st[N], tp, L[N], R[N];
signed main () {
  cin >> (ch + 1);  n = strlen(ch + 1);  SA();  Getheight();
  st[tp = 1] = 1;  FOR(i, 2, n) {
    while( tp && height[ st[tp] ] > height[i] ) R[ st[tp--] ] = i;
    L[i] = st[tp];  st[++tp] = i;
  }
  while (tp) R[ st[tp--] ] = n + 1; long long ans = n * (n - 1) * (n + 1) / 2;
  FOR(i, 2, n)  ans -= 2ll * (R[i] - i) * (long long) (i - L[i]) * height[i];
  cout << ans << "\n";
  return 0;
}
